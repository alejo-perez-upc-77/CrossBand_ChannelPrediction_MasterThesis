...
hist(c(train), breaks = 500, xlim = c(-0.3, .3), freq =FALSE, 
     main = 'Histogram of the training data (UE0)')
lines(x, hx,type = 'l', # density plot ,
      col = "red")
points((c(train)), yvals, cex=.25, col='green')

legend(x="topright", legend=c("MLE Student-T", "MLE Normal"),
...
hist(c(epsilon), breaks = 50, probability = TRUE, xlim=c(-.05,.05), main="Histogram of the Error of Reconstruction of PPCA (UE0)")
lines(x, hx,type = 'l', # density plot ,
      col = "red", cex=2)
...
hist(c(valid_pred[missing]), breaks = 400, probability = TRUE, main = 'Fitting distributions to predicted values', xlim=c(-.3,.3))
lines(x, hx,type = 'l', # density plot ,
      col = "red", cex=2)
...
hist(c(valid_pred[missing]- valid_unmask[missing]), breaks = 700, 
     probability = TRUE, main = 'Histogram of the Error of prediction of PPCA (UE0)', 
     xlim=c(-.3, .3))
