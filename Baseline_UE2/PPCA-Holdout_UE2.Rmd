...
fit_ppca <- function(train, Pcs=15, method="ppca"){
...
ppca_ <- fit_ppca(train, Pcs=15, method="ppca")
...
knitr::kable(table)
  ...
  plot(valid_unmask[missing], valid_pred[missing], col='blue',
       main="Hold-out Prediction PPCA UE2", ylab= 'Prediction', 
...
#
## BPCAs
bpca_ <-  pca(train, nPcs=15, method="bpca")
#
## Latent Varible aka Sigma in MVNorm
...
plot(valid_unmask[missing], valid_pred[missing], col='red', ylab= 'Prediction', 
     xlab= 'Background Truth', main="Hold-out Prediction BPCA UE2") 
...
for (k in 2:itNum){
  ...
  corrected_Abs_SE <- mean((valid_unmask[missing] - valid_pred[missing])^2)
