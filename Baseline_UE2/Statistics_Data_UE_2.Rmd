...
masked_data <- as.matrix(read.csv('../data_UE2_600/masked_data_UE2_600_04.csv', row.names = 1))
...
hist(c(train), breaks = 1000, xlim = c(-0.3, .3), freq =FALSE, 
     main = 'Histogram of the training data (UE2)')
...
hist(c(epsilon), breaks = 600, xlim=c(-.2,.2), probability = TRUE, main="Histogram of the Error of Reconstruction of PPCA (UE2)")
lines(x, hx,type = 'l', # density plot ,
      col = "red", cex=2)
...
hist(c(valid_pred[missing]- valid_unmask[missing]), breaks = 1500, probability = TRUE, main = 'Histogram of the Error of prediction error of PPCA (UE1)', xlim=c(-.4, .4))
lines(x, hx,type = 'l', # density plot ,
      col = "red", cex=2)
